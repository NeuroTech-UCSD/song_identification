...
clc; clear;
% Load data
load('data/attempt2data.mat','stim_p1_datavec','resp_p1_datamat','fs');

% Normalize data
stim_p1_datavec = sum(stim_p1_datavec,2);
resp_p1_datamat = resp_p1_datamat/std(resp_p1_datamat(:));

% Downsample data
fsNew = 64;
stim_p1_datavec = resample(stim_p1_datavec,fsNew,fs);
resp_p1_datamat = resample(resp_p1_datamat,fsNew,fs);
fs = fsNew;
...
nfold = 6;
testfold = 1;
[stimtrain,resptrain,stimtest,resptest] = mTRFpartition(stim_p1_datavec,resp_p1_datamat,nfold,...
    testfold);
